...
class ResumeWriteActivity : AppCompatActivity() {
    ...
    private fun saveDataAndGeneratePdf(
        uid: String, skills: String, projects: String, awards: String, gpa: String, cert: String, highlight: String
        ...
        try {
            withContext(Dispatchers.IO) {
                db.collection("users").document(uid).collection("resumes").add(resume).await()
            }
            ...
            val fileUri = withContext(Dispatchers.IO) {
                ResumePdfGenerator(this@ResumeWriteActivity).generateResumePdf(resume, userName)
            }
            ...
            Toast.makeText(this@ResumeWriteActivity, "이력서 생성 중 오류 발생: ${e.message}", Toast.LENGTH_SHORT).show()
            e.printStackTrace()
        }
    }
}